...
static adc_oneshot_chan_cfg_t chan_configs[MAX_ADC_UNITS][MAX_ADC_CHANNELS];
static bool channel_initialized[MAX_ADC_UNITS][MAX_ADC_CHANNELS] = {0};

#define RETURN_IF_ERROR(status, fmt, ...) \
    do { \
        if ((status) != ESP_OK) { \
            ESP_LOGE(TAG, fmt, ##__VA_ARGS__, esp_err_to_name(status)); \
            return; \
        } \
    } while (0)
...
{
    int unit_idx = unit;
    if(validate_adc_channel(unit, channel) != VALID) {
        ESP_LOGE(TAG, "Invalid ADC unit %d or channel %d", unit, channel);
        return;
    }
    ...
    // Set default config for this channel
    ...
    ESP_LOGI(TAG, "ADC unit %d channel %d configured", unit, channel);
}
...
int adc_read_channel(adc_unit_t unit, adc_channel_t channel)
{
    ...
    if(status != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read ADC unit %d channel %d: %s", unit, channel, esp_err_to_name(status));
        return INVALID_VALUE;
    }
...
void adc_deinit(adc_unit_t unit, adc_channel_t channel)
{
    ...
    ESP_LOGI(TAG, "ADC unit %d channel %d deinitialized", unit, channel);
    ...
    if (!any_channel_active && adc_handles[unit_idx]) {
        ESP_ERROR_CHECK(adc_oneshot_del_unit(adc_handles[unit_idx]));
        adc_handles[unit_idx] = NULL;
        ESP_LOGI(TAG, "ADC unit %d fully deinitialized", unit);
    }
}
